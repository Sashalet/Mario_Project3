...
int main()
{
	...
	while (window.isOpen())
	{
		...
		if (Keyboard::isKeyPressed(Keyboard::Up))
		{
			if (Mario.onGround) 
			{ 
				Mario.dy = -0.27; Mario.onGround = false;  sound.play();
			}
		...
		if (Mario.rect.intersects(enem.rect))
		{
			if (enem.life)
			{
				if (Mario.dy > 0) { enem.dx = 0; Mario.dy = -0.2; enem.life = false; }
				else Mario.sprite.setColor(Color::Red);
			}
		}
